...
# Define a threshold distance for alerting the user (in meters)
    ...
    for i in range(len(scores)):
        if ((scores[i] > min_conf_threshold) and (scores[i] <= 1.0)):
            ...
            if known_object_name in KNOWN_OBJECT_WIDTH:
                ...
                known_object_width_mm = None  # Or set a default value
            ...
            print(f"Distance to {label}: {distance_meters} meters")

            # Check if distance is within alert threshold and object is not already detected
            if distance_meters <= ALERT_DISTANCE_THRESHOLD and not object_detected:
